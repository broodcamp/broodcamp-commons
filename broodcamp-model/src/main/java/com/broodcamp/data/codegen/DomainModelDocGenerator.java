package com.broodcamp.data.codegen;
 ...
 */
public class DomainModelDocGenerator {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (File file : files) {
                ...
                // Parse class. If class is of no interest - does not have a table- do not set a
                // classname value and it will be disregarded
            ...
            // Append fields to a parent entity in case of Inheritance.SINGLE inheritance
            // strategy
        ...
        public void visit(ClassOrInterfaceDeclaration cd, DBTable dbTable) {
            ...
            if (cd.isAnnotationPresent("Table")) {
                ...
                for (ClassOrInterfaceType extendsFromType : cd.getExtendedTypes()) {
                    // System.out.println(dbTable.classname + " extends " +
                    // extendsFromType.getNameAsString());
                    ...
                    if (resolvedSuperClass.getTypeDeclaration() instanceof JavaParserClassDeclaration) {
                        ...
                        if (inheritanceAnnotation.isPresent()) {
                            ...
                            // In Joined type strategy parent entity stores fields in its own table. An ID
                            // field is added
                ...
                // Supplements a parent table with inheritance.SINGLE type of inheritance.
                // Fields will be added to parent table in later iteration.
                ...
                // Embedded entities. Fields will be added to entity holding embedded entity in
                // later interaction.
                ...
                if (fd.isTransient() || fd.isAnnotationPresent(Transient.class) || fd.isAnnotationPresent(OneToMany.class) || fd.isStatic() || fd.isFinal()) {
                    ...
                    if (variableType.getNameAsString().equals("Map")) {
                        ...
                        if (fd.isAnnotationPresent(AttributeOverrides.class)) {
                            ...
                            // Extract fields from referenced class and add comments, data types and default
                            // values to overriden column definitions
                    ...
                    if (fd.isAnnotationPresent(OrderColumn.class)) {
                        extraTable.fields.add(
                                new DBField("", getStringParameter(fd.getAnnotationByClass(OrderColumn.class).get(), "name"), "Integer", null, "Ordering number sequence", false));
                    }
                    return;
                    ...
                    try {
                        ...
                        // System.out.println("Failed to resolve fields for " + vd.getType() + " " +
                        // vd.getType().getClass());
                        // e.printStackTrace(System.out);
                        if (dbTable.failedEmbeddedTypes == null) {
    ...
    private static class DBTable {
        ...
        /**
         * Is this an embedded entity definition. In this case tablename will be null.
         * Fields will be appended to the entities where failedEmbeddedTypes match the
         * classname value of embedded entity.
         */
        public boolean isEmbedded = false;

        /**
         * Parent classname in case of Inheritance.SINGLE type inheritance. Fields will
         * be appended to the entity which classname match parentClassname value
         */
        ...
        /**
         * Provide a HTML text for document representing a table with fields and any
         * extra tables with their fields
         * 
         * @return HTML text
         */
        public String toHtml() {
            String html = "<style>table { font-family: arial, sans-serif; border-collapse: collapse; width: 100%; } td, th { border: 1px solid #dddddd; text-align: left; padding: 8px; } tr:nth-child(even) { background-color: #dddddd; }</style>";
            html += "<H1>" + (classname != null ? classname : "") + " (<b>" + tablename + "</b>)</H1><br/>" + (comment != null ? "<br/><p>" + comment + "</p>" : "");
